...
class RetrieverDataset(Dataset):
    def __init__(self, args, tokenizer, fold="train"):
        ...
        if args.file_name:
            with open(os.path.join(args.data_dir, args.file_name), 'r') as f:
             ...
             with open(os.path.join(args.data_dir, "GT_firstGen.json"), 'r') as f:
                self.original_dataset = json.load(f)
        ...
        if args.knowledge_base in ["wiki"]:
            print("Load Searcher...")
        ...
        if args.knowledge_base == "covid":
            print("Load Searcher...")
            self.searcher = LuceneSearcher.from_prebuilt_index('beir-v1.0.0-trec-covid.flat')
            print("Load Index...")
            self.index_reader = IndexReader.from_prebuilt_index('beir-v1.0.0-trec-covid.flat')
            self.vectorizer = BM25Vectorizer('beir-v1.0.0-trec-covid.flat')
    ...
    def parse_document(self, hit):
        ...
        if self.args.knowledge_base in ["Wikipedia", "covid"]:
            return hit.raw
        elif self.args.knowledge_base in ["pileoflaw", "raco"]:
            return json.loads(hit.contents())["contents"]
        elif self.args.knowledge_base in ["pubmed"]:
            return json.loads(hit.contents())["text"]
    ...
    def search(self):
        ...
        if not self.generate_search_space:
            ...
            for _data in tqdm(self.dataset, desc="Preprocess dataset.."):
                ...
                # question = data["first_gen"]
                # reasoning = data["ground_truth"]
                question = data[self.args.query_key]
                reasoning = data[self.args.rational_key]
                ...
                for i, hit in enumerate(silver_hits):
                    score = hit.score
                    docid = hit.docid
                    document = self.searcher.doc(docid).raw()
                    # print("document", document)
                    # print("self.parse_document(hit)", hit)
    ...
    def preprocess(self, data, space):
        # query = data["first_gen"]
        query = data[self.args.query_key]
